	...
	for (i=0; i< PROFILE_SIZE; i++) {
		...
		for (j=0; j*2 <= 100; j++) {
...
#ifdef SOC_BY_HW_FG
			{
				...
				scaled_dod = ((profile_p[i]+j)->percentage * 100)
					/ SOC_RESCALING_FACTOR;
				scaled_dod = scaled_dod - (100 - SOC_RESCALING_FACTOR);
				(profile_p[i]+j)->percentage = scaled_dod;
			}
#endif
#endif
		}
...
void dod_init(void)
{
	...
	if (g_boot_reason != BR_POWER_KEY) {
		gFG_capacity_by_v = gFG_capacity_by_v_init;
		bm_print(BM_LOG_CRTI, "[FGADC] use SW_OCV : HW_SOC=%d, SW_SOC = %d\n",
			gFG_capacity_by_v, gFG_capacity_by_v_init);
	}
#endif
	/* ------------------------------------------------------------------------------- */
...
#ifdef CONFIG_LGE_PM
#if defined(IS_BATTERY_REMOVE_BY_PMIC)
	sprintf(doddbg,"rtc:%d hwocv:(%d:%d) swocv:(%d,%d) battery_remove:%d %d %d %d\n",
		g_rtc_fg_soc,gFG_voltage,gFG_capacity_by_v,
		gFG_voltage_init,gFG_capacity_by_v_init,
		is_battery_remove_pmic(),
