...
static int battery_meter_set_profile(void)
{
	...
	case BATT_ID_SW3800_VC2:
		...
		break;
	default:
		bm_warn("[battery_meter_probe] Invalid battery attached.\n");
		break;
	}
	bm_info("[battery_meter_probe] battery_id = %d\n", id);
#endif
	return 0;
}
...
{
	bm_debug("__batt_meter_init_cust_data_from_cust_header\n");

	/* cust_battery_meter_table.h */
#ifdef CONFIG_LGE_PM_BATTERY_PROFILE
	batt_meter_table_cust_data.battery_profile_t0_size = battery_profile_size;
	memcpy(&batt_meter_table_cust_data.battery_profile_t0,
		battery_profile_t0,
		sizeof(BATTERY_PROFILE_STRUC)*battery_profile_size);

	batt_meter_table_cust_data.battery_profile_t1_size = battery_profile_size;
	memcpy(&batt_meter_table_cust_data.battery_profile_t1,
		battery_profile_t1,
		sizeof(BATTERY_PROFILE_STRUC)*battery_profile_size);

	batt_meter_table_cust_data.battery_profile_t2_size = battery_profile_size;
	memcpy(&batt_meter_table_cust_data.battery_profile_t2,
		battery_profile_t2,
		sizeof(BATTERY_PROFILE_STRUC)*battery_profile_size);

	batt_meter_table_cust_data.battery_profile_t3_size = battery_profile_size;
	memcpy(&batt_meter_table_cust_data.battery_profile_t3,
		battery_profile_t3,
		sizeof(BATTERY_PROFILE_STRUC)*battery_profile_size);

	batt_meter_table_cust_data.r_profile_t0_size = r_profile_size;
	memcpy(&batt_meter_table_cust_data.r_profile_t0,
		r_profile_t0,
		sizeof(R_PROFILE_STRUC)*r_profile_size);

	batt_meter_table_cust_data.r_profile_t1_size = r_profile_size;
	memcpy(&batt_meter_table_cust_data.r_profile_t1,
		r_profile_t1,
		sizeof(R_PROFILE_STRUC)*r_profile_size);

	batt_meter_table_cust_data.r_profile_t2_size = r_profile_size;
	memcpy(&batt_meter_table_cust_data.r_profile_t2,
		r_profile_t2,
		sizeof(R_PROFILE_STRUC)*r_profile_size);

	batt_meter_table_cust_data.r_profile_t3_size = r_profile_size;
...
#ifdef CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT
	/* charger will be enabled in battery_common.c */
#else /* MTK Original */
...
static int battery_meter_suspend(struct platform_device *dev, pm_message_t state)
{
...
#elif defined(SOC_BY_SW_FG) || defined(SOC_BY_HW_FG)
	{
		...
		bm_info("[battery_meter_suspend]2. sleep_total_time = %d, last_time = %d\n", sleep_total_time, last_time);
	}
#endif
...
//	bm_debug("[Netlink] fgd_cmd = %d, fgd_data_len = %d\n",msg->fgd_cmd, msg->fgd_data_len);
//	bm_debug("[Netlink] sizeof(struct fgd_nl_msg_t) %d\n", (int)sizeof(struct fgd_nl_msg_t));
	...
	switch (msg->fgd_cmd) {
		...
		case FG_DAEMON_CMD_GET_HW_OCV:
		{
			...
			if (g_boot_reason == BR_USB && !g_battery_soc_ready)
				voltage = gFG_voltage;
#endif
		...
		case FG_DAEMON_CMD_GET_CUSTOM_SETTING:
		{
			...
			//bm_debug(" FG_DAEMON_CMD_GET_CUSTOM_SETTING ret_msg->fgd_data_len %d\n",ret_msg->fgd_data_len);
		}
		break;
