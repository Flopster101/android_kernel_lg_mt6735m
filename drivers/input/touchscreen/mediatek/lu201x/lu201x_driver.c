...
//	mt_eint_registration ( CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, touch_eint_interrupt_handler, 1 );
...
static void touch_knock_suspend ( void )
{
	TPD_FUN ();
	int ret;
...
static void touch_knock_resume ( void )
{
	TPD_FUN ();
...
static int LU201x_touch_event_handler ( void *unused )
{
	...
	do
	{
		...
		if ( touch_data.EventType == EVENT_ABS )
		{
			for ( i = 0 ; i < touch_data.VPCount ; i++ )
			{
				...
				if ( info.status[i] == TYPE_PRESS )
				{
					...
					input_report_abs ( tpd->dev, ABS_MT_TOUCH_MAJOR, 50 );
					input_report_abs ( tpd->dev, ABS_MT_PRESSURE, 10 );
					input_mt_sync ( tpd->dev );
		...
		{
			TPD_ERR ( " Unknown Event occured from LU201x %d\n", touch_data.EventType );
			goto exit_work;
		}
...
static ssize_t show_sd ( struct device *dev, struct device_attribute *attr, char *buf )
{
	...
	ret += sprintf ( buf+ret, "FW_Version : %x.%x\n", fw_ver[0], fw_ver[1] );

	sd_write_log ( buf );
	msleep ( 30 );

	err = LU201x_read_register ( FW_STATUS_REG, &check, 1 );
	if ( err < 0 )
	{
...
static int LU201x_i2c_probe ( struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	err = LU201x_read_register ( LU201x_DEVICEID_ADDR, DeviceID, 2 );
	if ( err < 0 )
	{
		TPD_LOG ( "Touch IC is not LeadingUI\n" );
		//return -1;
	}
	else if ( ( DeviceID[1] != 0x20 ) || (( DeviceID[1] != 0x10 ) && ( DeviceID[0] != 0x20 )))
	{
...
//	SEL_SYSFS = 2;
...
#if 0
	err = LU201x_read_register ( FW_VERSION_REG, &fw_ver[0], 2 );
	if ( err < 0 )
	{
		TPD_ERR ( "FW_VERSION_REG read fail\n" );
	}
	...
	{
		TPD_LOG ( "Touch IC FW_Version [%x.%x]\n", fw_ver[0], fw_ver[1] );
		TPD_LOG ( "Binary FW_Version [%x.%x]\n", rawData[0x79FE], rawData[0x79FF] );

		/* Touch Firmware Update */
		if ( ( fw_ver[0] < rawData[0x79FE] ) || ( ( fw_ver[0] == rawData[0x79FE] ) && ( fw_ver[1] < rawData[0x79FF] ) ) )
		{
			TPD_LOG ( "Touch FW Update!! [%x.%x ==> %x.%x]\n", fw_ver[0], fw_ver[1], rawData[0x79FE], rawData[0x79FF] );
...
static int LU201x_i2c_remove ( struct i2c_client *client )
{
	TPD_FUN ();
	return 0;
}
...
static void LU201x_suspend ( struct early_suspend *h )
{
	...
	{
		288xx_eint_mask ( CUST_EINT_TOUCH_PANEL_NUM );
