...
#if 0
	mode = lge_get_boot_mode();
	if(mode == LGE_BOOT_MODE_CHARGERLOGO)
		mode = BOOT_OFF_CHARGING;
	else if(mode == LGE_BOOT_MODE_QEM_56K)
		mode = BOOT_MINIOS;
	else if(mode == LGE_BOOT_MODE_NORMAL)
		mode = BOOT_NORMAL;
	TOUCH_LOG("BOOT MODE = %d\n", mode);
#endif
	#else
...
int TouchRegisterIrq(TouchDriverData *pDriverData, irq_handler_t irqHandler)
{
	...
	mt_eint_registration(10, 2, irqHandler, 1);
