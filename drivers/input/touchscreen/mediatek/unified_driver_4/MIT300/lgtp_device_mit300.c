...
static const char defaultFirmware[] = "melfas/m2/melfas_mip4.bin";
#endif
...
static int MIT300_DoSelfDiagnosis(int* pRawStatus, int* pChannelStatus, char* pBuf, int bufSize, int* pDataLen)
{
	...
	*pChannelStatus = TOUCH_SUCCESS;
	...
	ret = MIT300_GetTestResult(client, pBuf, pChannelStatus, OPENSHORT2_SHOW);
	if (ret < 0) {
