...
static ssize_t show_version(TouchDriverData *pDriverData, char *buf)
{
	...
	WRITE_SYSBUF(buf, ret,	"Product ID - FW Version [%s - %X.%02X] \n", 
		(index == FIRST_MODULE) ? "Sn280h" : (index == SECOND_MODULE) ? "Ft6x36" : (index == THIRD_MODULE) ? "Lu202x" : "UNKNOWN"
...
static ssize_t store_mfts_lpwg_test(TouchDriverData *pDriverData, char *buf, size_t count)
{
	int ret = TOUCH_SUCCESS;
	int value = 0;
	TOUCH_FUNC();
