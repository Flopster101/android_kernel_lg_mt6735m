...
#else // JASON
static unsigned char CTPM_FW[] = {
	//#include "FT6x36_LGE_E1_Ver0x03_20150924_app.i"
};
			  ...
			  u32 dw_lenth)
{
	...
	if(pbt_buf[0] != 0x02)
	{
		TOUCH_LOG("FW first byte is not 0x02. so it is invalid \n");
		return -1;
	}
	...
	if(dw_lenth > 0x11f)
	{
		...
		{
			TOUCH_LOG("Fw length is invalid \n");
			return -1;
		}
	}
	else
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		msleep(FT6X06_UPGRADE_55_DELAY+delay_count);
		...
		if (reg_val[0] == FT6X06_UPGRADE_ID_1
			...
			//dev_dbg(&client->dev, "[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				//reg_val[0], reg_val[1]);
			TOUCH_LOG("Step 3: GET CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
	...
	for (j = 0; j < packet_number; j++) {
		...
		for(i = 0;i < 30;i++)
		{
			...
			{
				//TOUCH_LOG("write a block data finished \n");
				break;
			}
			msleep(1);
		}
	...
	TOUCH_LOG("Step 5-1:remain data write firmware(FW) to ctpm flash\n");
	/* Remain data. */
...
note:the firmware default path is sdcard.
	if you want to change the dir, please modify by yourself.
*/
static int ft6x06_ReadFirmware(char *firmware_name, unsigned char *firmware_buf)
{
	struct file *pfile = NULL;
	struct inode *inode;
	unsigned long magic;
...
static ssize_t ft6x06_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t ft6x06_tpfwver_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t ft6x06_tprwreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/*place holder for future use*/
	return -EPERM;
}

static ssize_t ft6x06_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	ssize_t num_read_chars = 0;
	int retval;
...
static ssize_t ft6x06_fwupdate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/* place holder for future use */
	return -EPERM;
}
...
/*upgrade from *.i*/
static ssize_t ft6x06_fwupdate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t ft6x06_fwupgradeapp_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char fwname[128];
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
struct proc_dir_entry *ft6x06_proc_entry;
/*interface of write proc*/

#if defined (KERNEL_VERSION_3_10)
...
static int ft6x06_debug_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
...
static int ft6x06_debug_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	struct i2c_client *client = (struct i2c_client *)ft6x06_proc_entry->data;
	int ret = 0;
...
int ft6x06_create_apk_debug_channel(struct i2c_client * client)
{
	...
	if (NULL == ft6x06_proc_entry) {
		TOUCH_ERR("Couldn't create proc entry!\n");
		return -ENOMEM;
	} else {
