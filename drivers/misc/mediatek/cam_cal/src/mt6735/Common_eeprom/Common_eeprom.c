...
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2015-10-20] ADD_MODULE_MAKER_CLASS_DRV
#if defined(DW9716_EEPROM)
	extern int DW9716_EEPROM_Ioctl(    struct file *file, unsigned int a_u4Command, unsigned long a_u4Param);
	extern uint16_t read_moduleID_DW9716(unsigned char * pinputdata);
	#define read_moduleID read_moduleID_DW9716
#endif
...
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2015-10-20] ADD_MODULE_MAKER_CLASS_DRV
/*******************************************************************************
*
********************************************************************************/
...
#endif
{
    ...
    {
#if defined(AT24C16D)
        EEPROMDB("[AT24C16D] Jump to IOCTL \n");
        i4RetValue = AT24C16D_EEPROM_Ioctl( file,a_u4Command,a_u4Param);
#else
        EEPROMDB("[AT24C16D] Not defined in config \n");
#endif
    }
    else
        {
...
inline static int RegisterEEPROMCharDrv(void)
{
	...
	struct device* camera_vendor_id_dev =NULL;
