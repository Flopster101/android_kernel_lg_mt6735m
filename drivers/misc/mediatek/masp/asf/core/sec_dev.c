...
#define ALT_BOOT_REGION0_PATH               "/dev/block/mmcblk1boot0"
...
int read_info (int part_index, uint32 part_off, uint32 search_region, char* info_name, uint32 info_name_len, uint32 info_sz, char* info_buf)
{
    ...
    {
		ASF_FILE fd;
		/* we don't know whether mmcblk0 or mmcblk1 is eMMC, we must try both mmcblk0boot0 and mmcblk1boot0 */
		...
		fd = ASF_OPEN(part_path);
		if (ASF_IS_ERR(fd)) {
			mcpy(part_path, ALT_BOOT_REGION0_PATH, strlen(ALT_BOOT_REGION0_PATH));
			part_path[strlen(ALT_BOOT_REGION0_PATH)] = '\0';
		}
		else
			ASF_CLOSE(fd);
    }
