...
#if defined(TARGET_MT6735P_M23G) // added by woonghwan.lee 150827 for M2 3G knock on sleep command
int disp_lcm_suspend_for_touch(disp_lcm_handle *plcm)
{
	DISPFUNC();
	LCM_DRIVER *lcm_drv = NULL;
	...
	{
		lcm_drv = plcm->drv;
		if(lcm_drv->suspend_2nd_cmd)
		{
			lcm_drv->suspend_2nd_cmd();
		}
		else
		{
			DISPERR("FATAL ERROR, lcm_drv->suspend_2nd_cmd is null\n");
			return -1;
		}
