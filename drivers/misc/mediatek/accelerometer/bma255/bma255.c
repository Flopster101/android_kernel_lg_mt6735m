...
static int BMA255_CheckDeviceID(struct i2c_client *client)
{
	...
	{
		SENSOR_ERR("BMA255_CheckDeviceID %d failt!\n ", databuf[0]);
		return BMA255_ERR_IDENTIFICATION;
	}
	else
	{
		SENSOR_LOG("BMA255_CheckDeviceID %d pass!\n ", databuf[0]);
	}
...
static int BMA255_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		SENSOR_LOG("set power mode ok: 0x%02x\n", databuf[1]);
	}
	//SENSOR_LOG("BMA255_SetPowerMode ok!\n");
...
static int BMA255_SetIntEnable(struct i2c_client *client, u8 intenable)
{
	...
	SENSOR_LOG("BMA255 interrupt was disabled\n");
...
static int bma255_init_client(struct i2c_client *client, int reset_cali)
{
	...
	if (client == NULL)
	{
		SENSOR_ERR("bma255_init_client client null!\n");
		return res;
	}
	...
	SENSOR_LOG("bma255_init_client OK!\n");
#ifdef CONFIG_BMA255_LOWPASS
...
static int BMA255_CompassReadData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(sensor_power == FALSE)
	{
		...
		if(res)
		{
			SENSOR_ERR("Power on bma255 error %d!\n", res);
		}
	}
	...
	else
	{
		...
		{
			SENSOR_LOG("gsensor data for compass: %s!\n", buf);
		}
	}
...
static int bma255_set_cal_trigger(struct i2c_client *client, unsigned char caltrigger)
{
	...
	if(atomic_read(&bma255->fast_calib_rslt) != 0)
	{
		...
		SENSOR_LOG(KERN_INFO "[set] bma2X2->fast_calib_rslt:%d\n",atomic_read(&bma255->fast_calib_rslt));
	}
...
static int bma255_calibration_save(int *cal)
{
   ...
   fd = sys_open(fname,O_WRONLY|O_CREAT|S_IROTH, 0666);
	if(fd< 0){
           SENSOR_LOG("[%s] File Open Error !!!(%d)\n", __func__,fd);
   		   sys_close(fd);
	       return -EINVAL;
		}
   for(i=0;i<6;i++)
   ...
   set_fs(old_fs);
   SENSOR_LOG("bma255_calibration_save Done.\n");
...
static int bma255_calibration_read(int *cal_read)
{
   ...
   for(i=0;i<6;i++)
   {
        ...
        res = sys_read(fd, temp_str, sizeof(temp_str));
         if(res<0){
   			 SENSOR_LOG("[%s] Read Error !!!\n", __func__);
              sys_close(fd);
              return -EINVAL;
         	}
        sscanf(temp_str,"%d",&cal_read[i]);
        SENSOR_LOG("bma255_calibration_read : cal_read[%d]=%d\n",i,cal_read[i]);
   	}
   sys_close(fd);
   set_fs(old_fs);
   SENSOR_LOG("bma255_calibration_read Done.\n");
...
static int make_cal_data_file(void)
{
   ...
   if(fd==-2)
   SENSOR_LOG("[%s] Open Cal File Error. Need to make file !!!(%d)\n", __func__,fd);
   
 	if(fd< 0){
	       fd = sys_open(fname,O_WRONLY|O_CREAT|S_IROTH, 0666);
	     if(fd< 0){
	       SENSOR_LOG("[%s] Open or Make Cal File Error !!!(%d)\n", __func__,fd);
  		   sys_close(fd);
	       return -EINVAL;
		   }
#if 1
	     if(LGE_FacReadAccelerometerCalibration((unsigned int*)cal_misc) == TRUE)
		     ...
		     SENSOR_LOG("Read Cal from misc Error !!!\n");
         	}
#endif
		     /* set default cal */   
			  if((cal_misc[0]==0)&&(cal_misc[1]==0)&&(cal_misc[2]==0))
			  	{
                   ...
                   cal_misc[2]=5;
                 #endif 
		           SENSOR_LOG("Default Cal is set : %d, %d, %d\n",cal_misc[0],cal_misc[1],cal_misc[2]);					  	
			  	}
    	      /* set default cal */   
...
static ssize_t store_cali_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	else if(3 == sscanf(buf, "%d %d %d", &offset_x, &offset_y, &offset_z))
	{
		...
		if(bma255_set_offset_z(bma255->client, (unsigned char)offsets[2]) < 0)
			return -EINVAL;
		SENSOR_LOG("store_cali_value success\n");
	}
	else
	{
...
static ssize_t bma255_fast_calibration_x_store(struct device_driver *ddri, char *buf, size_t count)
{
	...
	do{
		...
		SENSOR_LOG(KERN_INFO "x wait 2ms cal ready flag is %d\n", tmp);

		timeout++;
		if (timeout == 50)
		{
			SENSOR_LOG(KERN_INFO "get fast calibration ready error\n");
			return -EINVAL;
		}
	} while (tmp == 0);

	atomic_set(&bma255->fast_calib_x_rslt, 1);
	SENSOR_LOG(KERN_INFO "x axis fast calibration finished\n");
...
static ssize_t bma255_fast_calibration_y_store(struct device_driver *ddri, char *buf, size_t count)
{
	...
	do {
		...
		SENSOR_LOG(KERN_INFO "y wait 2ms cal ready flag is %d\n", tmp);
	...
	SENSOR_LOG(KERN_INFO "y axis fast calibration finished\n");
...
static ssize_t bma255_fast_calibration_z_store(struct device_driver *ddri, char *buf, size_t count)
{
	...
	do {
		...
		SENSOR_LOG(KERN_INFO " z wait 2ms cal ready flag is %d\n", tmp);
	...
	SENSOR_LOG(KERN_INFO "z axis fast calibration finished\n");
...
static int bma255_runCalibration(void)
{
	...
	{
		SENSOR_ERR("SetDataFormat 2G error");
	}
	...
	if (res2 < 0)//change range before calibration
	{
		SENSOR_ERR("SetDataFormat 4G error");
	}
	...
	if(res != BMA255_SUCCESS)
	{
		...
		if(bma255_set_offset_z(bma255->client, (unsigned char)backup_offset_z) < 0)
			return -EINVAL;
		SENSOR_LOG("Recovery backup cal value: x=%d, y=%d, z=%d\n", backup_offset_x, backup_offset_y, backup_offset_z);
...
static int bma255_do_calibration(void)
{
   ...
   /* set axis off set to zero */
	...
	SENSOR_LOG("===============complete shaking check===============\n");
	...
	SENSOR_LOG("===============complete zero-g check===============\n");
	...
	do{
		...
		SENSOR_LOG("x wait 2ms cal ready flag is %d\n", tmp);

		timeout++;
		if (timeout == 50)
		{
			SENSOR_LOG("get fast calibration ready error\n");
			return -EINVAL;
		}
	} while (tmp == 0);
	atomic_set(&bma255->fast_calib_x_rslt, 1);
	...
	do {
		...
		SENSOR_LOG("y wait 2ms cal ready flag is %d\n", tmp);
	...
	do {
		...
		SENSOR_LOG(" z wait 2ms cal ready flag is %d\n", tmp);
...
static ssize_t bma255_runCalibration_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(res == BMA255_SUCCESS)
	{
		SENSOR_LOG("self calibration BMA255_SUCCESS\n");
		selfCalibration = BMA255_SUCCESS;
	}
	else if(res == BMA255_ERR_SETUP_FAILURE)
	{
		SENSOR_ERR("self calibration BMA255_ERR_SETUP_FAILURE\n");
		...
		selftest)
{
	...
	SENSOR_LOG("selftest_st comres : %d\n",comres);
	return comres;
}
...
static int bma255_set_selftest_stn(struct i2c_client *client, unsigned char stn)
{
	...
	comres = bma255_smbus_write_byte(client, BMA255_NEG_SELF_TEST__REG,
			&data);
	//comres = i2c_master_send(client, &data, 1);
	SENSOR_LOG("selftest_stN comres : %d\n",comres);
	return comres;
}
...
static ssize_t bma255_SelfTest_store(struct device_driver *ddri, char *buf, size_t count)
{
    ...
    if(data == 1)// self test start command
    {
		...
		{
			SENSOR_ERR("I2C error: ret value=%d", error);
			return EIO;
		}
		else
		{
		...
		SENSOR_LOG("self test finished. result : %d\n",result);
    }
	else // wrong input
	{
    	SENSOR_ERR("SelfTest command FAIL\n");
    	return -EINVAL;
	}
	return count;
}
...
static ssize_t bma255_SelfTest_show(struct device_driver *ddri, char *buf)
{
    ...
    struct bma255_i2c_data *obj = i2c_get_clientdata(client);
	if (obj == NULL) {
		SENSOR_ERR("bma i2c data pointer is null\n");
		return 0;
	}
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			//LGE_CHANGE_S : 2012-12-07 taebum81.kim@lge.com AT%SURV : (6)
				...
				if(err!=0){
				    SENSOR_LOG("Read Cal Fail from file !!!\n");
					break;
					}
				else
					{
            ...
            if((sensor_data.x==0)&&(sensor_data.y==0)&&(sensor_data.z==0))
             {
			    SENSOR_LOG("Read Cal Data x : %d / y : %d / z : %d\n",sensor_data.x,sensor_data.y,sensor_data.z);
			    SENSOR_LOG("Read Cal Data all Zero, Do not set register\n");
				err = -EINVAL;
				break;
             }
...
static int bma255_resume(struct i2c_client *client)
{
	...
	{
		SENSOR_ERR("initialize client fail!!\n");
		return err;
	}
	#endif
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		SENSOR_ERR ( "failed to init BMA255 ( err = %d )\n", err );
	...
	{
		SENSOR_ERR("bma255_device register failed\n");
...
static int bma255_i2c_remove(struct i2c_client *client)
{
	...
	{
		SENSOR_ERR("bma150_delete_attr fail: %d\n", err);
	}
...
static void async_sensor_init(void *data, async_cookie_t cookie)
{
	...
	SENSOR_LOG("i2c_number = %d / bma255_init success\n\n", hw->i2c_num);
	return;
}
