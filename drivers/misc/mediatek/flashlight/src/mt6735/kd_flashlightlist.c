...
extern void rt8542_forced_flash_off();//LGE_CHANGE: [2015-10-06] kyhunghun.oh@lge.com, forced flash off when camera on with volume key
extern void release_rt8542_forced_flash_off();//LGE_CHANGE: [2015-10-06] kyhunghun.oh@lge.com, forced flash off when camera on with volume key
//==============================
// functions
//==============================
int globalInit(void)
...
static int flashlight_open(struct inode *inode, struct file *file)
{
	...
	rt8542_forced_flash_off();//LGE_CHANGE: [2015-10-06] kyhunghun.oh@lge.com, forced flash off when camera on with volume key
	if(bInited==0)
	{
		globalInit();
		bInited=1;
...
static int flashlight_release(struct inode *inode, struct file *file)
{
	...
	release_rt8542_forced_flash_off();//LGE_CHANGE: [2015-10-06] kyhunghun.oh@lge.com, forced flash off when camera on with volume key
	checkAndRelease();
...
#if 0//LGE_CHANGE: [2015-11-16] kyhunghun.oh@lge.com, flash on enven if battery is under 15%
