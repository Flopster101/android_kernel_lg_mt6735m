...
#if defined(CONFIG_LGE_HANDLE_PANIC) || defined(CONFIG_LGE_HIDDEN_RESET) || defined (CONFIG_LGE_ONE_BIN_MEMORY) /* ONE_BIN_MEMORY */
...
//#endif
	...
	printk("[mt_devs] [%s] lge_add_panic_handler_devices success\n", __func__);
	...
	printk("[mt_devs] [%s] lge_add_hidden_reset_devices success\n", __func__);
...
static void display_early_memory_info(void)
{
	...
	#if defined (CONFIG_LGE_ONE_BIN_MEMORY) /* ONE_BIN_MEMORY */
	printk("[ONE_BIN_MEMORY] kernel_mem_sz : 0x%llx\n", kernel_mem_sz);
	g_bl_mem_sz = kernel_mem_sz;
	...
	printk("[ONE_BIN_MEMORY] LGE_BSP_MEM_MAX_PHY_ADDR : 0x%lx\n", LGE_BSP_MEM_MAX_PHY_ADDR);
	#endif
	#endif
}
...
unsigned int get_total_DRAM_size(void)
{
	...
	#if defined (CONFIG_LGE_HANDLE_PANIC) || defined (CONFIG_LGE_HIDDEN_RESET)
	total_size = (g_bl_mem_sz + LGE_BSP_RESERVED_MEM_SIZE + LGE_BSP_LK_MEM_SIZE);
	#else
	total_size = g_bl_mem_sz;
	#endif

	printk("[ONE_BIN_MEMORY] Kernel Total DRAM size = 0x%x\n", total_size);
