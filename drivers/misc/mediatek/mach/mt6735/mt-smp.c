...
#if 1 /* jjm_t32 : Default Off. Set on this value at Trace32 for normal CPU hotplug working */
atomic_t trace32_on = ATOMIC_INIT(0);
#endif
...
int __cpuinit mt_smp_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
    ...
    #if 1 /* jjm_t32 : Sequential Sync between SMP and Trace32 */
    if(atomic_read(&trace32_on) == 1)
        atomic_set(&trace32_on, 1);
    #endif
