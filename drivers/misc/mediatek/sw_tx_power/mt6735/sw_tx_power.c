...
{
	pr_debug("[swtp] swtp_mod_eint_read_timer [cnt[%ld]\n", data);
	swtp_mod_eint_read();
	...
	while (1) {
		...
		for (i = 0; i < SWTP_CTRL_MAX_STATE; i++) {
			...
			if (swtp_state_reg[i].mode == SWTP_SUPER_MODE)
				...
				pr_err("[swtp error]: need to check 0x%x 0x%x\n", i, swtp_state_reg[i].mode);
		}
		mutex_unlock(&swtp_ctrl_lock);
...
{
	pr_debug("[swtp] swtp_mode_update_handler  md_id[%d], data[%d]\n", md_id, data);
...
static int __init swtp_mod_init(void)
{
	...
	pr_debug("[swtp_tr] running swtp thread\n");
