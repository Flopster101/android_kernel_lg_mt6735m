...
extern int dw9714_af_current_on; //[LGE_UPDATE][kyunghun.oh@lge.com][2015-10-26] DECREASE_AF_TIC_NOISE
...
static int AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    LOG_INF("Start \n");
	//[LGE_UPDATE_S][kyunghun.oh@lge.com][2015-10-26] DECREASE_AF_TIC_NOISE
	if(dw9714_af_current_on == 0)
	{
		LOG_INF("AF_POWER_NOT_INITIALIZED_AT_OPEN!\n");
		return 0;
	}
	//[LGE_UPDATE_E][kyunghun.oh@lge.com][2015-10-26] DECREASE_AF_TIC_NOISE
    if(g_s4AF_Opened)
...
static int AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	if(dw9714_af_current_on == 0)
	{
		LOG_INF("AF_POWER_NOT_INITIALIZED_AT_RELEASE!\n");
		return 0;
	}
