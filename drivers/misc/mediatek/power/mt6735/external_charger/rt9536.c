...
{
	mt_set_gpio_out(info->en_set | 0x80000000, high);
}

static int rt9536_get_en_set(struct rt9536_info *info)
{
	return mt_get_gpio_in(info->en_set | 0x80000000);
}

static int rt9536_get_chgsb(struct rt9536_info *info)
{
	return mt_get_gpio_in(info->chgsb | 0x80000000);
}

static int rt9536_gpio_init(struct rt9536_info *info)
{
	mt_set_gpio_mode(info->chgsb | 0x80000000, GPIO_MODE_GPIO);
	mt_set_gpio_dir(info->chgsb | 0x80000000, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(info->chgsb | 0x80000000, GPIO_PULL_DISABLE);

	mt_set_gpio_mode(info->en_set | 0x80000000, GPIO_MODE_GPIO);
	mt_set_gpio_dir(info->en_set | 0x80000000, GPIO_DIR_OUT);
	if (!rt9536_get_chgsb(info))
		mt_set_gpio_out(info->en_set | 0x80000000, GPIO_OUT_ZERO);
...
static int rt9536_parse_dt(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(np, "rt9536,en_set" ,&info->en_set);
	if (rc) {
		dev_err(&pdev->dev, "en_set not defined.\n");
		return rc;
	}

	rc = of_property_read_u32(np, "rt9536,chgsb" ,&info->chgsb);
	if (rc) {
		dev_err(&pdev->dev, "chgsb not defined.\n");
		return rc;
	}

	rc = of_property_read_u32(np, "rt9536,pgb" ,&info->pgb);
	if (rc) {
		/* just warning */
		dev_warn(&pdev->dev, "pgb not defined.\n");
	}

	rc = of_property_read_u32(np, "rt9536,iset" ,&info->iset);
	if (rc) {
		/* just warning */
		dev_warn(&pdev->dev, "iset not defined.\n");
	}
