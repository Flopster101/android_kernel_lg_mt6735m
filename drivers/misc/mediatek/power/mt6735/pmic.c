...
{
	PMICLOG("[vbaton_undet_int_handler]BATON_UNDET status = %d....\n",pmic_get_register_value(PMIC_RGS_BATON_UNDET));

	/* this is level interrupt, so we should disable first */
	pmic_enable_interrupt(35,0,"PMIC");

	/* wakeup bat_thread_kthread */
	wake_up_bat3();
}
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	pmic_register_interrupt_callback(35,vbaton_undet_int_handler);
#endif
...
#ifdef CONFIG_LGE_PM_BATTERY_PRESENT
	pmic_enable_interrupt(35,1,"PMIC");
#endif
...
#ifdef CONFIG_LGE_PM
	pmic_set_register_value(PMIC_RG_UVLO_VTHL, 0); // Set UVLO VTHL 2.5V
#endif
}
