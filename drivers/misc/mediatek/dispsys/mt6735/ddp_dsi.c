...
void DSI_set_cmdq_V3(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_setting_table_V3 *para_tbl, unsigned int size, unsigned char force_update)
{
	...
	do {
		...
		if (0 != DSI_REG[0]->DSI_MODE_CTRL.MODE){//not in cmd mode
			DSI_VM_CMD_CON_REG vm_cmdq;
			DSI_OUTREG32(cmdq,&vm_cmdq, AS_UINT32(&DSI_REG[0]->DSI_VM_CMD_CON));
			...
			if (count > 1)
			{
				...
				vm_cmdq.CM_DATA_0 = count+1;
				DSI_OUTREG32(cmdq,&DSI_REG[0]->DSI_VM_CMD_CON, AS_UINT32(&vm_cmdq));

				goto_addr = (UINT32)(&DSI_VM_CMD_REG[0]->data[0].byte0);
				mask_para = (0xFF<<((goto_addr&0x3)*8));
				set_para = (cmd<<((goto_addr&0x3)*8));
				DSI_MASKREG32(cmdq,goto_addr&(~0x3), mask_para, set_para);
				...
				for(i=0; i<count; i++)
				{
					goto_addr = (UINT32)(&DSI_VM_CMD_REG[0]->data[0].byte1) + i;
					mask_para = (0xFF<<((goto_addr&0x3)*8));
					set_para = (para_list[i]<<((goto_addr&0x3)*8));
					DSI_MASKREG32(cmdq, goto_addr&(~0x3), mask_para, set_para);
				}
...
int ddp_dsi_set_lcm_utils(DISP_MODULE_ENUM module, LCM_DRIVER *lcm_drv)
{
	...
	if(module == DISP_MODULE_DSI0)
	{
		...
		utils->dsi_set_cmdq_V3	= DSI_set_cmdq_V3_Wrapper_DSI0; //sangcheol.seo@lge.com 20150709
	...
	else if(module == DISP_MODULE_DSI1)
	{
		...
		utils->dsi_set_cmdq_V3	= DSI_set_cmdq_V3_Wrapper_DSI1; //sangcheol.seo@lge.com 20150709
	...
	else if(module == DISP_MODULE_DSIDUAL)
	{
		...
		else
		{
			...
			utils->dsi_set_cmdq_V3	= DSI_set_cmdq_V3_Wrapper_DSIDual; //sangcheol.seo@lge.com 20150709
...
.dsi_set_cmdq_V3 = DSI_set_cmdq_V3_Wrapper_DSI0 //sangcheol.seo@lge.com 20150709
};
