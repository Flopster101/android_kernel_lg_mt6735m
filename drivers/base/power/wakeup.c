					    ...
					    struct wakeup_source *ws)
{
	...
	if (ktime_to_ms(active_time) > 0)
		ret = seq_printf(m, "%-12s\t%lu\t\t%lu\t\t%lu\t\t%lu\t\t%lu\t\t"
				"%lld\t\t%lld\t\t%lld\t\t%lld\t\t%lld\n",
				ws->name, active_count, ws->event_count,ws->relax_count,
				ws->wakeup_count, ws->expire_count,
				ktime_to_ms(active_time), ktime_to_ms(total_time),
				ktime_to_ms(max_time), ktime_to_ms(ws->last_time),
				ktime_to_ms(prevent_sleep_time));
...
{
	struct wakeup_source *ws;
	seq_puts(m, "name\t\tactive_count\tevent_count\trelax_count\twakeup_count\t"
		"expire_count\tactive_since\ttotal_time\tmax_time\t"
		"last_change\tprevent_suspend_time\n");
	rcu_read_lock();
	list_for_each_entry_rcu(ws, &wakeup_sources, entry)
...
#ifdef CONFIG_MACH_LGE
	wakeup_sources_active_stats_dentry = debugfs_create_file("wakeup_sources_active",
